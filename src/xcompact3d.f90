...
subroutine write_params_json
   ...
   if (nrank .eq. 0) then
      ...
      write(fl,"(A ,' : {')") '  "geometry"'
      write(fl,"(A ,': ',ES23.16,',')") '    "xlx"',xlx
      write(fl,"(A ,' : ',ES23.16,',')") '    "yly"', yly
      write(fl,"(A ,': ',ES23.16)") '    "zlz"', zlz
      write(fl,*) "  },"

      write(xfmt,'(A,I0,A)') "( A, ': [',ES23.16,",nx-1,"(',',ES23.16),'],')"
      write(yfmt,'(A,I0,A)') "( A, ': [',ES23.16,",ny-1,"(',',ES23.16),'],')"
      write(zfmt,'(A,I0,A)') "( A, ': [',ES23.16,",nz-1,"(',',ES23.16),']')"
