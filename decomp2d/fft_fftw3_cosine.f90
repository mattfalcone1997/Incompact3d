...
subroutine c2c_1m_x_plan(bcx,plan1, decomp, isign)

  implicit none

  integer, intent(in) :: bcx
  ...
  if (bcx ==0) then
    ...
    if (isign.eq.FFTW_FORWARD) then
      call dfftw_plan_many_r2r(plan1,1,decomp%xsz(1), &
                              decomp%xsz(2)*decomp%xsz(3),a1,&
                              decomp%xsz(1), 1, decomp%xsz(1), a1,&
                              decomp%xsz(1), 1, decomp%xsz(1), &
                              FFTW_REDFT10,plan_type)
    else
      call dfftw_plan_many_r2r(plan1,1,decomp%xsz(1), &
                              decomp%xsz(2)*decomp%xsz(3),a1,&
                              decomp%xsz(1), 1, decomp%xsz(1), a1,&
                              decomp%xsz(1), 1, decomp%xsz(1), &
                              FFTW_REDFT01,plan_type)
    endif
  else if (bcx == 2) then
    call dfftw_plan_many_r2r(plan1,1,decomp%xsz(1), &
                            decomp%xsz(2)*decomp%xsz(3),a1,&
...
elseif (bcx==1) then
  ...
  subroutine c2c_1m_y_plan(bcy,plan1, decomp, isign)

    implicit none
    integer, intent(in) :: bcy
    integer*8, intent(OUT) :: plan1
    TYPE(DECOMP_INFO), intent(IN) :: decomp
    integer, intent(IN) :: isign
    ...
    if (bcy ==0) then
      ...
      if (isign.eq.FFTW_FORWARD) then
        call dfftw_plan_many_r2r(plan1,1,decomp%ysz(2), &
              decomp%ysz(1),a1, decomp%ysz(2),decomp%ysz(1) , 1, a1,&
  ...
  if (bcy ==0) then
    ...
    call sfftw_plan_many_r2r(plan1,1,decomp%ysz(2), &
            decomp%ysz(1),a1, decomp%ysz(2),decomp%ysz(1) , 1, a1,&
            decomp%ysz(2), decomp%ysz(1), 1, &
            FFTW_REDFT11,plan_type)
  endif
  ...
  subroutine c2c_1m_z_plan(bcz,plan1, decomp, isign)

    implicit none
    integer,intent(in) :: bcz
    integer*8, intent(OUT) :: plan1
    TYPE(DECOMP_INFO), intent(IN) :: decomp
    integer, intent(IN) :: isign
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine init_fft_engine(bcx,bcy,bcz)

    implicit none
    integer, intent(in), optional :: bcx, bcy, bcz 
    if (nrank==0) then
       write(*,*) ' '
       write(*,*) '***** Using the FFTW (version 3.x)  cosine engine *****'
       write(*,*) ' '
    end if
    ...
    if (format == PHYSICAL_IN_X) then
       ...
       ! For C2C transforms
       call c2c_1m_x_plan(bcxx,plan(-1,1), ph, FFTW_FORWARD )
       call c2c_1m_y_plan(bcyy,plan(-1,2), ph, FFTW_FORWARD )
       call c2c_1m_z_plan(bczz,plan(-1,3), ph, FFTW_FORWARD )
       call c2c_1m_z_plan(bczz,plan( 1,3), ph, FFTW_BACKWARD)
       call c2c_1m_y_plan(bcyy,plan( 1,2), ph, FFTW_BACKWARD)
       call c2c_1m_x_plan(bcxx,plan( 1,1), ph, FFTW_BACKWARD)

       ! For R2C/C2R tranforms
       call r2c_1m_x_plan(plan(0,1), ph, sp)
       call c2c_1m_y_plan(bcyy,plan(0,2), sp, FFTW_FORWARD )
       call c2c_1m_z_plan(bczz,plan(0,3), sp, FFTW_FORWARD )
       call c2c_1m_z_plan(bczz,plan(2,3), sp, FFTW_BACKWARD)
       call c2c_1m_y_plan(bcyy,plan(2,2), sp, FFTW_BACKWARD)
       ...
       call c2c_1m_y_plan(bcyy,plan(0,2), sp, FFTW_FORWARD )
       call c2c_1m_x_plan(bcxx,plan(0,1), sp, FFTW_FORWARD )
       call c2c_1m_x_plan(bcxx,plan(2,1), sp, FFTW_BACKWARD)
       call c2c_1m_y_plan(bcyy,plan(2,2), sp, FFTW_BACKWARD)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine c2c_1m_x(inout, isign, plan1)
    ...
    real(mytype), dimension(sp%xsz(1),sp%xsz(2),sp%xsz(3)) :: rl, im
    integer, intent(IN) :: isign
    ...
    if (bcxx.eq.0) then
      ...
      call dfftw_execute_r2r(plan1, inout%re, rl)
      call dfftw_execute_r2r(plan1, inout%im, im)

      inout = cmplx(rl,im,kind=mytype)
    endif
#else
    if (bcxx.eq.0) then
      call sfftw_execute_dft(plan1, inout, inout)
    else
      rl = real(inout,kind=mytype)
      im = aimag(inout)
      call sfftw_execute_r2r(plan1, rl, rl)
      call sfftw_execute_r2r(plan1, im, im)
      inout = cmplx(rl,im,kind=mytype)
    endif
#endif
  ...
  subroutine c2c_1m_y(inout, isign, plan1)
    ...
    complex(mytype), dimension(:,:,:), intent(INOUT) :: inout
    real(mytype), dimension(sp%ysz(1),sp%ysz(2)) :: rl, im
    ...
    s3 = size(inout,3)
      ...
      if (bcyy.eq.0) then
        ...
        do k=1,s3
          call dfftw_execute_r2r(plan1, inout(:,:,k)%re, rl)
          call dfftw_execute_r2r(plan1, inout(:,:,k)%im, im)
    
          inout(:,:,k) = cmplx(rl,im,kind=mytype)
        enddo
      endif
#else
  if (bcyy.eq.0) then
    ...
    do k=1,s3
      call sfftw_execute_r2r(plan1, inout(:,:,k)%re, rl)
      call sfftw_execute_r2r(plan1, inout(:,:,k)%im, im)
